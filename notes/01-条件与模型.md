...
> ⚠️ **实测把这个预期推翻了，而且原因在数据不在模型。** 在第一版（确定性）数据上，
> 回归 SemAcc 98.5% / MPJPE 3.86 cm，flow 只有 64.2% / 11.21 cm。
> 因为那份数据给定 (文本, 语音) 之后动作几乎唯一（条件变异只有 3.37 cm），
> 回归已经贴着噪声底了。「多对多」是一个**关于数据的经验性质**，不是定理。
> 全过程和修法见 [`notes/05`](05-第一环-任务设计的坑.md)。
...
MPS 上 batch 32 × 窗口 120 帧，实测 **2.56 步/秒**，8000 步 52 分钟。
