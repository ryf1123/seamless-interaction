...
第一次真的用上它（`v3b_bandlimited`，5000 步、每 1000 步测一次）：

```
训练中的验证集 SemAcc：1000:49%  2000:51%  3000:41%  4000:46%  5000:38%
   → best_sem.pt 选中了第 2000 步

测试集上：
   按 val loss 选的 best.pt      SemAcc 62.0 %
   按 SemAcc  选的 best_sem.pt   SemAcc 56.9 %   ← 更差
```

**按 SemAcc 选反而选到了更差的检查点。**

原因是**验证集太小**：12 条句子 ≈ 40 个语义事件，二项标准误约 8 个百分点。
那串 49 / 51 / 41 / 46 / 38 基本是噪声，看不出趋势——
在噪声里挑最大值，等于随机挑一个，还带上了「挑最大值」的正偏。

所以这一条要分开说：

- **「val loss 和 SemAcc 会反向走」这个现象是真的**，
  在 `v2_best` 上是 val loss 降 25% / SemAcc 掉 13 个百分点，量级足够大、不是噪声。
- **「所以按 SemAcc 选检查点」这个对策，按我现在的实现是不成立的**——
  验证集的 SemAcc 估计噪声（±8 个百分点）比要检测的差异还大。

要让它真正可用，至少得：把验证句数提到 40+（代价是每次约 50 秒）、
或者对多个种子取平均、或者对曲线做平滑再挑。
**在那之前，`best_sem.pt` 不该被当成默认选择。** 已在 `si/train.py` 的注释里写明。
