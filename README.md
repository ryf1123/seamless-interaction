...
python -m si.train --config configs/flow_body_2k.yaml --name best \
       --set basis_k=48 ema=0.999 steps=12000
python -m si.eval --run runs/best
```

全部在 2000 句数据、**200 条测试句 / 664 个语义事件**上评测：

| | **SemAcc ★** | 95% 区间 | FGD | MPJPE | 抖动 / 真值 |
|---|---|---|---|---|---|
| token + EMA | 70.3 % | [66.8, 73.8] | **0.130** | 10.02 cm | 16.43× |
| ↑ + 推理后 SG 滤波 | 76.1 % | — | 15.663 | 7.43 cm | 4.36× |
| **↑↑ 换成学习基 K=48** | **82.2 %** | **[79.2, 84.9]** | 0.358 | 8.03 cm | 6.01× |
| 学习基 + 再加事后滤波 | 80.1 % | — | 0.331 | **7.23 cm** | **3.07×** |

配对 McNemar（同一批 664 个事件）：学习基比 token+EMA **+11.9 个百分点，132 比 53，p < 0.0001**。

两条值得注意：
- **学习基把事后滤波吃掉了**——在 2000 句上再加滤波反而掉 2.1 个百分点（82.2 → 80.1），
  因为投影本身已经提供了平滑，再滤一道就过头了。
- **FGD 从 15.663 变成 0.358（好 44 倍）**：事后滤波是把训好的输出硬掰平滑，
  在自编码器隐空间里就是「被弄坏了」；学习基让模型从一开始就住在子空间里，
  生成的分布本身是对的。
...
`bow` 和 `shuffle` 是 9 比 9，完美对称。

2000 句上（200 条测试句 / 664 个事件、四组同为 8000 步）确认了这一点，
并给出更诚实的上界：`seq` 配对 **+61.3 个百分点**（431 比 24，p<0.0001），
`bow` **+2.1**（p=0.10，不显著但方向一致），`shuffle` −0.2（p=1.0000）。
**即使 `bow` 那 2.1 点是真的，也只占 `seq` 的 3.4 %。**
...
| **函数类（学习基）** | 用训练集学时间轴正交基，噪声/目标/输出都投到前 K 维 | ✅ **SemAcc +11.9（p<0.0001）、抖动 −63%、FGD 好 44 倍**——全项目唯一同时改善两者的做法 |
