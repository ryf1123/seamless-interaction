...
| 组 | 携带的信息 | **SemAcc** |
|---|---|---|
| `seq` 逐帧对齐的词 id | 说了什么 ✅ + 什么时候说 ✅ | **66.4 %** [59.7, 73.1] |
| `bow` 整句词袋 | 说了什么 ✅ + 什么时候说 ❌ | 16.8 % [11.6, 22.2] |
| `shuffle` 句内换位 | 说了什么 ❌ + 什么时候说 ✅ | 12.4 % [7.6, 18.1] |
| `none` 没有文本 | 都没有 | 13.1 % [8.3, 18.1] |
...
顺带一个独立证据说明 BeatAlign 没用：四组的节拍对齐是 0.74 / 0.76 / 0.77 / 0.78，
几乎完全一样，而 SemAcc 从 66.4% 掉到 13.1%。
...
| 1 | 生成式 vs 确定性（flow matching vs L1 回归） | ⚠️ 一测：回归完胜（98.5% vs 64.2%），但那是**任务设计**的问题——数据几乎是确定性的；已造多峰版数据并重跑，见 [notes/05](notes/05-第一环-任务设计的坑.md) |
| 2 | **文本到底有没有在起作用**（seq / shuffle / bow / none） | ✅ seq **66.4%** vs bow 16.8% / shuffle 12.4% / none 13.1%（随机 7.7%）——[notes/04](notes/04-第二环-文本到底有没有用.md) |
