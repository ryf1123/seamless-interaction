...
随机基线 7.7%，真值上限 100%。**后三组的置信区间互相重叠、统计上不可区分**——
...
换成多峰数据后排序翻转（flow 34.1% vs 回归 23.2%，但 CI 重叠）。
...
真值上限 0.929。三组**全部落在随机水平**——这和 GENEA Challenge 2023
在真实数据、12 支队伍规模上的结论一致（「对话者适配性最好也就勉强高于随机」）。

但一个**不依赖点头检测器**的诊断显示信息其实部分流进来了：
生成的颈部俯仰与对方语音包络的相关，AV 组 0.066（真值 0.076），
只给对方语音的组 0.008，只给自己语音的组 −0.005。
**信息只从对方的「动作」进来，没从「原始语音」进来。**
...
| **输出本身** | Savitzky-Golay 滤波，窗口 9（300 ms） | **降 73%，MPJPE 降 22%，SemAcc +2.9** |

前两条排除了「抖动来自随机性」，第四条效果好一个量级——
