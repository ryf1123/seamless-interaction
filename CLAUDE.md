...
- **任何指标进主表之前，先回答三个问题**（这个项目栽过三次）：
  ...
  2. 上限是多少（把真值喂进去）？
  3. 要比的两组，置信区间会不会重叠？
- **同测试集上的两组一律做配对检验**，不要只看各自的区间重不重叠。
  非配对等于扔掉「两组做的是同一批题目」这个信息，统计效率差一到两个数量级。
  二元结果用 `si.report` 的 `mcnemar()`（精确检验），连续量用配对自助。
- **长跑进程用的是它启动那一刻的代码。**（这个坑栽过三次：FGD 自编码器缓存 shape mismatch
  让两轮训练白跑；`si.ablate` 进程里的旧 `si.eval` 少写了 `per_event` 字段导致配对比较做不了。）
  改了 `si/` 里的东西之后，**要么等当前套件跑完再改，要么改完把受影响的 run 重评一遍**。
- **开跑之前把预期写进 PLAN.md 的「预注册预测」**，结果出来逐条对账，
  被推翻的原文保留。不这么做的话，人总能给任何结果编一个事后解释。
